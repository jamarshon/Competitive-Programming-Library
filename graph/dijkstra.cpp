...
class Graph {unordered_map<T, vector<Edge<T>>> vertex_to_edges_; bool directed_; public: Graph(const bool& directed = false) {directed_ = directed; } void AddVertex(const T& id) {assert(vertex_to_edges_.find(id) == vertex_to_edges_.end() && "Vertex already exists"); vertex_to_edges_[id] = vector<Edge<T>>{}; } void AddEdge(const T& from, const T& to, const int weight = 0) {auto from_edges_it = vertex_to_edges_.find(from); auto to_edges_it = vertex_to_edges_.find(to); assert((from_edges_it != vertex_to_edges_.end() && to_edges_it != vertex_to_edges_.end()) && "Edge already exists"); (from_edges_it -> second).emplace_back(to, weight); if(!directed_) {(to_edges_it -> second).emplace_back(from, weight); } }
    ...
    pair<unordered_map<T, int>, unordered_map<T, T>> GetShortestPath(const T& source) {
        ...
        while(!edges_to_process.empty()) {
            ...
            for(Edge<T> neighbor: neighbors) {
                auto edge_set_it = edges_to_process.find({neighbor.to, min_distance[neighbor.to]});
    ...
    vector<T> RecreatePath(const T source, const T dest, unordered_map<T, T>& parent) {
        vector<T> res;
        for(T i = dest; i != source; i = parent[i]) {
            res.push_back(i);
        }
    ...
    pair<unordered_map<T, int>, unordered_map<T, vector<T>>> GetAllShortestPaths(const T& source) {
        assert(vertex_to_edges_.find(source) != vertex_to_edges_.end() && "Source vertex not found");
        unordered_map<T, vector<T>> parents;
        set<Edge<T>> edges_to_process;
        unordered_map<T, int> min_distance;
        ...
        while(!edges_to_process.empty()) {
            ...
            for(Edge<T> neighbor: neighbors) {
                ...
                // found a smaller distance
                if(current_weight + neighbor.weight == min_distance[neighbor.to]) {
                    parents[neighbor.to].push_back(current_vertex);
                } else if(current_weight + neighbor.weight < min_distance[neighbor.to]) {
                    min_distance[neighbor.to] = current_weight + neighbor.weight;
                    parents[neighbor.to] = {current_vertex};
    ...
    void RecreateDFS(const T source, const T curr, unordered_map<T, vector<T>>& parents,
        vector<T>& temp, vector<vector<T>>& res) {
        if(curr == source) {
            vector<int> temp_rev(temp.rbegin(), temp.rend());
            res.push_back(temp_rev);
            return;
        }

        for(auto from: parents[curr]) {
            temp.push_back(from);
            RecreateDFS(source, from, parents, temp, res);
            temp.pop_back();
        }

    }

    vector<vector<T>> RecreateAllPaths(const T source, const T dest, unordered_map<T, vector<T>>& parents) {
        vector<vector<T>> res;

        vector<T> temp{dest};
        RecreateDFS(source, dest, parents, temp, res);
        return res;
    }
};
...
void TestGetShortestPath1() {
    ...
    tie(distance, parent) = g.GetShortestPath('A');

    vector<char> output_path = g.RecreatePath('A', 'F', parent);
    ...
    vector<char> expected_path{'A', 'E', 'F'};
    ...
    cout << "Run path test" << endl;
    int N = output_path.size();
    assert(N == expected_path.size());
    for(int i = 0; i < N; i++) {
        assert(expected_path[i] == output_path[i]);
    }
    cout << "Pass path test" << endl;
}
...
void TestGetShortestPath2() {
    ...
    tie(distance, parent) = g.GetShortestPath(0);
    ...
    tie(distance, parents) = g.GetAllShortestPaths(0);
    vector<vector<int>> output_paths = g.RecreateAllPaths(0, 8, parents);

    cout << "Run all shortest distance test" << endl;
    assert(expected_distance.size() == distance.size());
    for(auto d: distance) {
        assert(expected_distance[d.first] == d.second);
    }
    cout << "Pass all shortest distance test" << endl;

    cout << "Run all shortest path test" << endl;
    int N = output_paths.size();
    assert(N == expected_paths.size());
    for(int i = 0; i < N; i++) {
        int M = output_paths[i].size();
        assert(M == expected_paths[i].size());
        for(int j = 0; j < M; j++) {
            assert(output_paths[i][j] == expected_paths[i][j]);
        }
    }
    cout << "Pass all shortest path test" << endl;
}
} // namespace GraphTest
