...
public:
    ...
    void AddEdge(const T& from, const T& to, const int weight = 0) {
        auto from_edges_it = vertex_to_edges_.find(from);
        auto to_edges_it = vertex_to_edges_.find(to);
        assert((from_edges_it != vertex_to_edges_.end() && to_edges_it != vertex_to_edges_.end()) && "Edge already exists");
        (from_edges_it -> second).emplace_back(to, weight);
        if(!directed_) {
            (to_edges_it -> second).emplace_back(from, weight);
        }
    }
};
